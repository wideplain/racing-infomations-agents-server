...
**Android アプリはピットで使う、声を拾う側の道具です。** ピットの会話を端末内で文字起こしし、テキストだけをサーバーに送って解析を回します。解析モードの選択肢が「通常 / ピットウォール」の2つなのはそのためで、ドライバー向けの短い指示は選択肢ではなく、設定の「ドライバー要約」で同じ発話からもう一本だけ別に走らせます。車内には置きません（位置情報を送る機能も持っていません）。
...
この分担は、サーバー側の解析3モード（`default` / `pitwall` / `driver`）とそのまま対応します。ピットのアプリが `default` か `pitwall` を選んで投げ、ビュワーはその2つをタイムラインに並べます。`driver` は車内向けなので、アプリが裏で併走させ、ドライバー画面だけがその結果を出します。
