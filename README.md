...
```
        ピット                                車内
  Pixel アプリ（オンデバイスSTT）        ブラウザ（ドライバー表示）
  ブラウザ（ビュワー）                    解析を見る / GPS を送る
        │                                        │
        └──────────────── Tailscale ─────────────┘
                              │
              Mac (Fastify + SQLite) → codex exec → 解析結果を返却
```
...
- **AI解析の4モード**
  ...
  - **質問応答モード**: クルーの質問に、会話ログとこれまでの記録だけを根拠に答える（追随質問に対応できるよう、ピットウォールの記録も対象に含む）。根拠にした発言を引用し（`basedOn`）、答えられない点は推測せず「記録にありません」と返す。引用の時刻はサーバー側で実際のログ行と突き合わせ、一致しないものは実時刻に変換せず「ログと一致せず」と表示する（存在しない時刻を、確かめられる根拠のように見せないため）。曖昧な答えの方が、答えが出ないことより危険という考え方による。参照する会話ログの範囲も他モードの40件・8,000字より広い400件・40,000字にしてあり、記録に残っている話を見逃して「記録にありません」と答えてしまう事態を避けている
...
| | どこで使うか | 読むもの | 書くもの | 本番で使うか |
|---|---|---|---|---|
| **Android アプリ** | **ピット** | 解析結果 | セッション、文字起こし、解析要求 | **○ 実際の入力経路** |
| **ビュワー** `/viewer.html` | ピット | 文字起こし、解析タイムライン全体、ルート、天気 | **なし** | **○** |
| **ドライバー** `/driver.html` | **車内** | 最新のドライバー解析、天気 | この端末の GPS | **○** |
| **テキストテスト** `/index.html` | 開発者の手元 | すべて | セッション、発話、解析、終了 | **× 開発用** |

ピット側に2つあるのは、役割が違うためです。**Android アプリが声を拾う側**で、ピットの会話を文字起こしし、解析を投げます。**ビュワーは見る側**で、同じセッションを別の画面から眺めます。車内にあるのはドライバー画面だけで、これが GPS を送ります。

「ピット」はどちらも指します。Android アプリもピットの道具で、ビュワーを PC で開いたときの統合表示も画面名が「ピット」です。どちらもピットに置くものなので、名前を分けていません。区別が要るときは「ピットのアプリ」「ビュワーのピット表示」と呼び分けてください。

**Android アプリはピットで使う、声を拾う側の道具です。** ピットの会話を端末内で文字起こしし、テキストだけをサーバーに送って解析を回します。解析モードの選択肢が「通常 / ピットウォール」の2つなのはそのためで、ドライバー向けの短い指示は選択肢ではなく、設定の「ドライバー要約」で同じ発話からもう一本だけ別に走らせます。質問応答も選択肢には加えず、「📝 メモを添えて解析」の隣に「❓ 質問する」という独立したボタンを置いています。レース中にモードを都度切り替えるのは遅すぎるためです。車内には置きません（位置情報を送る機能も持っていません）。
...
**ビュワーは「見るための画面」です。** POST・PUT・PATCH・DELETE を一切発行しません。だからこそ、走行中ずっとピットのモニタに開きっぱなしにしておいても、進行中のセッションに触れてしまう操作がそもそも画面上に存在しません。同じピットでも、アプリが喋る側の端末で、ビュワーは眺めるための別画面という分担です。幅 1024px 以上ではタブがなくなり、文字起こし・解析タイムライン・地図・これからの雨が同時に並びます。切り替えずに一目で見たいためです。
...
この分担は、サーバー側の解析4モード（`default` / `pitwall` / `driver` / `question`）とそのまま対応します。ピットのアプリが `default` か `pitwall` を選んで投げ、ビュワーはその2つをタイムラインに並べます。`driver` は車内向けなので、アプリが裏で併走させ、ドライバー画面だけがその結果を出します。`question` はピットのアプリが「❓ 質問する」から投げ、ビュワーとテキストテストがタイムラインに表示します。ドライバー画面には出しません。
...
- **100m 走るごとに1点**記録します。距離は直前の**記録点**から測るため、ゆっくりした GPS のふらつきが積み上がって1点になることはありません
- ただし**停車中も5分に1度は記録**します。天気の各エンドポイントは10分より古い位置情報を受け付けないため、これがないと停まった瞬間から天気表示が消えてしまうためです
- **タイムスタンプは送信端末基準**です。GPS フィックスの時刻が `recorded_at` に入り、サーバーが受け取った時刻は別カラムの `created_at` に入ります。電波が悪くて送信が遅れても、記録時刻は走行時の時刻のまま残ります
- 速度表示には**すべてのフィックス**を使います。100m 間隔は速度を求めるには粗すぎるため、間引くのは記録だけです
- 記録された点は、表示側でさらに用途ごとに間引かれます（区間時刻履歴は 250m ごと、地名解決は 300m 未満の点を無視）
...
5. 会話ログから確かめたいことがあれば **「❓ 質問する」** で尋ねる。会話ログと記録だけを根拠に答え、わからない点は「記録にありません」と返る
6. 別端末のブラウザでビュワーを開けば、同じ内容を並走して確認できる。ピット側は PC で開いて「ピット」表示、運転席では `driver.html` を開く
